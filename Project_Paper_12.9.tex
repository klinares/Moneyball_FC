...
break for analysis. We measure market value using Transfermarkt
estimates, validated as a robust proxy for actual transfer fees (Herm et
al., 2014). Existing models typically treat players as static units
nested within single clubs, ignoring that players transfer between teams
and accumulate value from both portable human capital and institutional
club prestige---a cross-classified structure is particularly important
when club-level pandemic shocks may differentially affect valuations. We
...
``Boom-and-Bust'' cycle. Age is grand-mean centered (\(\bar{A} = 26.5\)
years) with a quadratic term to model the biological career arc. Minutes
played is standardized within each club-year to measure squad status
independent of team rotation policies, while goals and assists are
position-centered to capture performance relative to role-specific
expectations. The vectors \(\mathbf{D}^{pos}_i\) and
\(\mathbf{D}^{reg}_i\) contain dummy variables for player position
(reference: Attacker) and region of origin (reference: Europe \& Central
Asia), respectively. Citizenship status (\(\text{Citizen}_{ij}\))
indicates whether player \(i\) holds citizenship in club \(j\)'s
country.

\subsection{Centering Strategies}\label{centering-strategies}
...
pre-pandemic inflation, post-pandemic correction).\footnote{Performance
  expectations vary by role: A goal from a striker is routine while for
  a defender is exceptional. Position-centering transforms raw counts
  into ``performance above expectation'' metrics, ensuring the model
  captures relative contributions rather than treating goals and assists
  equally.}
...
C. Here we output our final model's variance components and fixed
effects.
...
\midrule
\textbf{Year (linear)} & $\gamma_1$ & 0.002 & 0.030 & 0.08 & 0.2 & \\
\textbf{Year²} & $\gamma_2$ & -0.270 & 0.024 & -11.05 & -23.7 & ***\\
\textbf{Year³} & $\gamma_3$ & -0.117 & 0.011 & -10.98 & -11.0 & ***\\
\textbf{Height} & $\beta_1$ & 0.010 & 0.004 & 2.81 & 1.0 & **\\
\textbf{Defender} & $\beta_{pos,1}$ & -0.403 & 0.058 & -6.97 & -33.2 & ***\\
\textbf{Goalkeeper} & $\beta_{pos,2}$ & -1.059 & 0.095 & -11.17 & -65.3 & ***\\
\textbf{Midfielder} & $\beta_{pos,3}$ & -0.190 & 0.058 & -3.29 & -17.3 & **\\
\textbf{Domestic citizen} & $\beta_2$ & -0.185 & 0.031 & -5.89 & -16.9 & ***\\
\textbf{Age (linear)} & $\gamma_4$ & 0.019 & 0.004 & 5.02 & 1.9 & ***\\
\textbf{Age²} & $\gamma_5$ & -0.024 & 0.000 & -63.08 & -2.4 & ***\\
\textbf{Goals} & $\gamma_7$ & 0.005 & 0.002 & 2.36 & 0.5 & *\\
\textbf{Assists} & $\gamma_8$ & 0.010 & 0.003 & 3.51 & 1.0 & ***\\
\textbf{Red cards} & $\gamma_9$ & 0.009 & 0.022 & 0.42 & 0.9 & \\
\textbf{Yellow cards} & $\gamma_{10}$ & -0.002 & 0.003 & -0.49 & -0.2 & \\
\textbf{Minutes (Z)} & $\gamma_6$ & 0.350 & 0.013 & 26.20 & 41.9 & ***\\
\textbf{UEFA Coefficient} & $\gamma_{12}$ & 0.000 & 0.001 & -0.39 & 0.0 & \\
\textbf{Proportion of Foreign Players} & $\gamma_{11}$ & 0.103 & 0.104 & 0.99 & 10.8 & \\
\bottomrule
\end{tabular}
...
Diagnostic plots: (A) Q-Q plot for player random intercepts, (B) Q-Q
plot for club random intercepts, (C) residuals versus fitted values.
...
Correlations among club-level random effects (intercept, linear slope,
quadratic slope).
