...
goals as drivers for player valuation (Frick, 2007), more recent
scholarship highlights non-performance factors such as brand visibility
(Franck \& Nüesch, 2012; Rosen, 1981). However, the market has recently
bifurcated into a pre-2020 hyper-inflationary era, termed the ``Neymar
Effect'' (Poli et al., 2017), and a post-2020 pandemic correction
(Drewes et al., 2021), with COVID-19 causing an estimated €8.7 billion
revenue shortfall (Deloitte, 2021) that serves as a natural structural
break for analysis. Using Transfermarkt estimates as validated proxies
for transfer fees (Herm et al., 2014), we fit a cross-classified
multilevel model to account for players transferring between clubs. We
address this gap by fitting a cross-classified multilevel model to a
decade of data (2013--2024), centering at the 2020 baseline to isolate
pandemic impacts while accounting for player movements across clubs. We
attempt to disentangle individual reputation from club context by
investigate four critical d

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  What non-linear ``Boom and Bust'' (i.e., rapid economic expansion met
  with sharp contraction) patterns characterize the transfer market's
  evolution relative to the 2020 pandemic baseline?
\item
  At what age does player market valuation reach its peak across the
  biological career trajectory?
\item
  To what degree does squad status---defined by relative playing
...
further descriptive statistics on each variable see Appendix A, Table 1.
...
modeling player and club as crossed random effects rather than nested
...
The log-transformed market value for player \(i\) at club \(j\) in
season \(t\) is modeled as:
...
role-specific expectations.\footnote{Performance expectations vary by
  role: A goal from a striker is routine while for a defender is
...
Appendix C, Table 1. Our final model converged successfully and model
diagnostic plots (see Appendix C, Figure 1) confirmed normality of
random effects and homoscedastic residuals, along with the correlation
structure captured in Appendix C, Figure 2.
...
pre-2019 ``Boom'' into a structural correction. Appendix C, Figure 3
displays club specific trajectories, with panels ordered by baseline
prestige. Elite clubs maintain higher valuations throughout, while
trajectory curvature varies substantially. Clubs like Barcelona and
Juventus show concave patterns indicating stagnation, whereas Arsenal
and Liverpool display more resilient recovery, indicating a more
resilient recovery and active value generation throughout the decade.
...
value. We visualize in Figure 3 the biological constraints on valuation.
...
\includegraphics{Project_Paper_12.9_files/figure-pdf/unnamed-chunk-5-1.pdf}
...
add little beyond club random effects. Figure 4 summarizes Level 2 fixed
...
reasonably satisfied---player-level random intercepts adhered closely to
normality, while minor tail departures in club-level effects were
expected given only 21 Level 2 units. The residuals versus fitted values
plot showed homoscedastic variance with no systematic patterns,
...
Additionally, the Latin American premium indicates continued market
inefficiencies that selling clubs in South America might leverage in
transfer negotiations. As the market continues to recover from
...
\subsection*{Figure 1. Model Diagnostic
Plots}\label{figure-1.-model-diagnostic-plots}
\addcontentsline{toc}{subsection}{Figure 1. Model Diagnostic Plots}

\includegraphics{Project_Paper_12.9_files/figure-pdf/unnamed-chunk-10-1.pdf}

Figure 1 presents diagnostic plots assessing model assumptions. The Q-Q
plots confirmed that player-level random intercepts (\(n=2,159\))
...
Hox, 2004). The residuals versus fitted values plot (Panel C) reveals
homoscedastic variance with no systematic patterns, supporting the
model's distributional assumptions.

\newpage

\subsection*{Figure 2. Club Random Effects Correlation
Structure}\label{figure-2.-club-random-effects-correlation-structure}
\addcontentsline{toc}{subsection}{Figure 2. Club Random Effects
Correlation Structure}

\includegraphics{Project_Paper_12.9_files/figure-pdf/unnamed-chunk-11-1.pdf}

Figure 2 displays the correlation structure among club-level random
effects. The non statistically significantly negative correlation
between intercept and linear slope (r = -0.063) indicates that clubs
with higher baseline prestige experienced slower subsequent growth---a
regression-to-the-mean pattern where elite clubs had less room for
appreciation while smaller clubs showed steeper trajectories. The
positive correlation between linear and quadratic terms (r = 0.749)
suggests that clubs with faster initial growth also experienced more
pronounced deceleration around the pandemic period.

\newpage

\subsection{Figure 3. Club-Specific Market Value Trajectories
(2013-2024)}\label{figure-3.-club-specific-market-value-trajectories-2013-2024}

\includegraphics{Project_Paper_12.9_files/figure-pdf/unnamed-chunk-12-1.pdf}

Figure 3 displays predicted market value trajectories for each of the 21
clubs, ordered by baseline prestige. The vertical positioning of panels
confirms the club hierarchy, with elite clubs (Manchester United,
Manchester City, Chelsea) maintaining consistently higher valuations
than selling clubs (Sevilla, Benfica, Porto). The curvature of
individual trajectories reveals heterogeneity in pandemic response:
clubs like Barcelona and Juventus show concave patterns indicating
stagnation, while Arsenal and Liverpool display convex trajectories
suggesting more resilient recovery.
