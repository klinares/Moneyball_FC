...
boxplot(usd_revenue_2024_log ~ club_id,
        data = clean_player_df,
        xlab = "Club ID",
        ylab = "Log revenue (2024 USD)",
        main = "Distribution of log revenue by club",
        las = 2)  # rotate x-axis labels so you can read them
...
ggplot(clean_player_df) +
  geom_line(aes(x = season_year_c, y = usd_revenue_2024_log, 
              group = club_id),color = factor(clean_player_df$club_id), alpha = 0.5)
...
-    After adding cubic terms, it yields a better fit. It cannot only explain the peak of the overall trajectory, but can also explain the up and down wave pattern over time of the trajectory.

```{r}

season_year_c_cub <- clean_player_df$season_year_c^3

Model_cub <-  lmer(
  usd_revenue_2024_log ~ 
    # first level fixed effect
    season_year_c + season_year_c_sq + season_year_c_cub +
    # player fixed effects
    height_c + position + citizen_of_club_country + Region +
    age_c + age_c_sq + 
    goals + assists + red_cards + yellow_cards + minutes_played +
