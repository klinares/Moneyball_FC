...
pacman::p_load(arrow, gt, ggthemes, zoo, patchwork,  lubridate, countrycode,
               priceR, tidyverse)
...
# for the exchange rate API, make a free api key 
# https://exchangerate.host/dashboard
api_key <- read_csv("D:/repos/api-keys.csv") |> 
  filter(key_id == "exchangerate.host") |> 
  pull(key)

Sys.setenv("EXCHANGERATEHOST_ACCESS_KEY"=api_key)
```
...
team_urls_df |>
  ...
  select(Country, Total_Pts, club_id) |> 
...
players_appearance <- read_parquet(str_c(
  ...
  mutate(year_month = zoo::as.yearmon(date),
         year_played = year(date)) |> 
  filter(club_id %in% Clubs$club_id) |> 
  # create season dates
...
player_df <- players_appearance |> 
  ...
  # bring in club names
  left_join(Clubs |> select(club_id, Club, Country))
  

  
```

We use the priceR package to convert euros to dollars, and take the yearly average from 2013-2024. Following the exchange rate conversion, we have to adjust for inflation. We use the same package and for each row US nominal dollar (values of the dollar for the specific year) we convert to real dollars (value of the dollar in a constant year) in our case to the most recent year of 2024. The below code chunk makes request to the exchange API and is a long process. The below code chunk makes request to the exchange API and is a long process.
...
yearly_exchange_rates <- tibble(
  season_year = as.double(seq(2013, 2024)),
  year_ex_rate = c(
    1.33, 1.33, 1.11, 1.11, 1.13, 1.18, 1.12, 1.14, 1.18, 1.05, 1.08, 1.08)
)
  
player_df <- player_df |> 
  left_join(yearly_exchange_rates) |> 
  mutate(market_value_ave_usd = market_value_ave * year_ex_rate) |> 
  mutate(
    # Create the new column
    usd_revenue_2024 = adjust_for_inflation(
      price = market_value_ave_usd, # The nominal dollar amount to convert
      from_date = season_year,    # The year the money was from
      country = "US",      # The country for the inflation index
      to_date = 2024       # The target year you want to convert to
    )
  ) 
```
...
We plot the proportion of the players that have have been in C number of clubs, with almost 80% being only in 1 club during their career.
...
player_df |> 
  ...
  geom_col(alpha=.5, fill="dodgerblue") +
  scale_y_continuous(labels = scales::percent) +
  labs(x="Clubs a player has been a member of", y="Percent of Players ") +
  theme_hc()
```

Next, we explore the average market value over time and can see a lot of variability, with some players valued very high. The first plot are in euros followed by US dollars in plot 2. The third plot is in US 2024 dollars adjusted for inflation and we can see slight changes from nominal values. The final plot is in log 2024 US dollars, which spreads lower salary values.
...
p_2 <-  player_df |> 
  ggplot(aes(x = season_year, y = market_value_ave_usd)) +
  geom_jitter( alpha = 0.75, width = .60, size = .50) +
  stat_summary(fun = mean, geom = "line", alpha=.8,
              linewidth = 1.5, color="dodgerblue") + 
  scale_x_continuous(breaks = player_df$season_year, 
  ...
  labs(subtitle = 
         "Blue line: Average Season Market Value.",
       x = "Season", y = "Market Value (in USD)") +
  theme_hc()
...
p_4 <-  player_df |> 
  ggplot(aes(x = season_year, y = log(usd_revenue_2024))) +
  geom_jitter( alpha = 0.75, width = .60, size = .50) +
  stat_summary(fun = mean, geom = "line", alpha=.8,
              linewidth = 1.5, color="dodgerblue") + 
  scale_x_continuous(breaks = player_df$season_year, 
...
Below we plot each team's average yearly market value in 2024 USD and on the log scale (second graph) and find that most teams have some growth but perhaps were impacted during the pandemic time frame more than other teams. We can see that on the log scale, for instance Man City, the trend line gets slightly smoothed out compared to the raw scale.
...
p_1 <- player_df |> 
  ...
  reframe(ave_yearly_market_value = mean(usd_revenue_2024)) |>
  ...
  labs(y="Market Value in 2024 USD ", x="Season") +
  facet_wrap(~Club) +
  theme_hc()

p_2 <- player_df |> 
  group_by(Club, season_year) |> 
  reframe(ave_yearly_market_value = log(mean(usd_revenue_2024))) |> 
  ggplot(aes(x=season_year, y=ave_yearly_market_value)) +
  geom_line() +
  scale_y_continuous(labels = scales::dollar) +
  scale_x_continuous(breaks=seq(2013, 2024, 4)) +
  labs(y="Market Value in 2024 USD log", x="Season") +
  facet_wrap(~Club) +
  theme_hc()
...
We plot age by yearly market 2024 USD value (on the log scale) for each team and observe almost an upside down u shape. The average overall age was 26.5 (SD=4.58) among these players. As you get older you are valued less in the market, same for younger players.
...
club_predictors <- player_df |> 
  ...
  reframe(foreign_players = 
            mean(ifelse(country_of_birth != Country, 
                                             1, 0 ), na.rm=TRUE)) |> 
  # join eufa coefficients
  left_join(team_urls_df |> 
              select(club_id, season_year, uefa_coefficient))
...
### There are too many countries represented in the dataframe in terms of country of birth

```{r}
player_df |> distinct(country_of_birth) |> count()

```

-   We can use the countrycode package to group countries into regions.

```{r}
#| echo: FALSE

# First this package does not recognize england or wales as country, but part of the UK
## we will need to recode some countries

country_of_birth_list <- player_df |> 
  distinct(country_of_birth) |> 
  mutate(country_of_birth_fixed = case_when(
    country_of_birth == "England" ~ "United Kingdom",
    country_of_birth == "Wales" ~ "United Kingdom",
    country_of_birth == "Scotland" ~ "United Kingdom",
    country_of_birth == "Northern Ireland" ~ "United Kingdom",
    country_of_birth == "CSSR" ~ "Czechoslovak Socialist Republic",
    country_of_birth == "UdSSR" ~ "Union of Soviet Socialist Republics",
    country_of_birth == "Jugoslawien (SFR)" ~ "Yugoslavia",
    country_of_birth == "Réunion" ~ "France",
    .default = as.character(country_of_birth)
  )) |> 
  # get the regions for each country
  mutate(Region = countrycode(country_of_birth_fixed, 
                               origin="country.name", destination="region")) 

# merge regions to the player_df
player_df <- player_df |> left_join(country_of_birth_list)

player_df |> 
    distinct(player_id, Region) |>
    group_by(Region) |> 
    count() |> 
    drop_na() |> 
    ungroup() |> 
    mutate(prop = n/sum(n), Region = fct_reorder(Region, prop, max)) |> 
    ggplot(aes(x=Region, y=prop, fill=Region)) +
    geom_col() +
    coord_flip() +
    scale_y_continuous(labels = scales::percent) +
    scale_fill_viridis_d(option = "B", end=.85) 
  
  
```

-   We also want to create a binary variable that tells us if the player is a citizen of the country they play in.

```{r}
player_df <- player_df |> 
  mutate(citizen_of_club_country = ifelse(country_of_citizenship==Country, 
                                          "Yes", "No")) 

player_df |> 
  group_by(player_id) |> 
  distinct(citizen_of_club_country) |> 
  ungroup() |> 
  count(citizen_of_club_country) |> 
  drop_na() |> 
...
-   **Level 3: The Contextual Group.** This is the **club**. Players are nested within clubs, and club-level characteristics (like the proportion of foreign players or the country the club plays in) can influence player value.
    ...
    -   New: We added UEFA yearly coefficients for each team.

\

The final dataframe is named player_df.parquet.