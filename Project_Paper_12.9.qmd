...
Figure 2 displays the correlation structure among club-level random effects. The non statistically significantly negative correlation between intercept and linear slope (r = -0.063) indicates that clubs with higher baseline prestige experienced slower subsequent growth i.e., a regression-to-the-mean pattern where elite clubs had less room for appreciation while smaller clubs showed steeper trajectories. The positive correlation between linear and quadratic terms (r = 0.749) suggests that clubs with faster initial growth also experienced more pronounced deceleration around the pandemic period.
